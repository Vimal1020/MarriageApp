...
WORKDIR /src
COPY ["MarriageAppLambda.csproj", "MarriageAppLambda/"]
RUN dotnet restore "MarriageAppLambda/MarriageAppLambda.csproj"

WORKDIR "/src/MarriageAppLambda"
COPY . .
RUN dotnet build "MarriageAppLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MarriageAppLambda.csproj" \
...
Execute unit tests
```
    cd "MarriageAppLambda/test/MarriageAppLambda.Tests"
    dotnet test
```

Deploy application
```
    cd "MarriageAppLambda/src/MarriageAppLambda"
